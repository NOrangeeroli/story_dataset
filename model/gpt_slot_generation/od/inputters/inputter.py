...
from od.inputters.dataset_wb import WBDataset, WBDataset2,WBdistDataset
...
def get_data(tokenizer, dataset_path, dataset_cache, logger):
    ...
    print("faq ",os.getcwd(),os.path.isfile(dataset_cache),dataset_cache)
    if dataset_cache and os.path.isfile(dataset_cache) and 1==0:
        logger.info("Load tokenized dataset from cache at %s", dataset_cache)
        dataset = torch.load(dataset_cache)

        #with open("/home/liuziqi/test/CDial-GPT/data/dataset.json", "w",encoding='utf-8') as fout:
