...
class WBDataset(Dataset):
    ...
    def __init__(self, data, tokenizer, max_history=15, batch_first=True, lm_labels=True):
        ...
        self.pad = -100#tokenizer.pad_token_id
    ...
    def process(self, history, resposne, with_eos=True):
        ...
        bos, eos = self.tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS)
        sequence = [[bos]] + history + [resposne + ([eos] if with_eos else [])]
        sequence = [sequence[0]] + [s for i, s in enumerate(sequence[1:])]
        instance = {}
        ...
        if self.lm_labels:
            ...
            sequence = [[bos]] + [[-100] * len(list(chain(*history)))] + [resposne + ([eos] if with_eos else [])]
            sequence = [sequence[0]] + [s for i, s in enumerate(sequence[1:])]
            instance["lm_labels"] = list(chain(*sequence))
...
class WBDataset2(Dataset):
    ...
    def process(self, history, resposne, with_eos=True):
        ...
        if self.lm_labels:
            #instance["lm_labels"] = ([-100] * sum(len(s) for s in sequence[:-1])) + [-100] + sequence[-1][1:]
            sequence = [resposne + ([eos] if with_eos else [])]
            sequence = [sequence[0]] + [s for i, s in enumerate(sequence[1:])]
            instance["lm_labels"] = list(chain(*sequence))
            r'''for i in range(len(instance["lm_labels"])):
                if instance["lm_labels"][i]==-1:
                    instance["lm_labels"][i]=-100'''
...
class WBdistDataset(DatasetBase):
    ...
    def process(self, history, resposne, with_eos=True):
        ...
        if self.lm_labels:
            instance["lm_labels"] = ([-100] * sum(len(s) for s in sequence[:-1])) + [-100] + sequence[-1][1:]
            for i in range(len(instance["lm_labels"])):
                if instance["lm_labels"][i]==-1:
                    instance["lm_labels"][i]=-100
