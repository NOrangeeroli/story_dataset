...
class AutoclozeGrabberGenGpt(AutoclozeGrabber):
    def __init__(self,data_path):
        temp = pd.read_csv(data_path)
        temp['context'] = temp['context'].apply(lambda x: x.replace('<MASK>','[MASK]'))
        temp['source'] = temp.context
        ...
        self.valid_data = self.data[self.data.context.isin(c_valid)].sample(frac=1).reset_index(drop=True)
        self.test_data = self.data[self.data.context.isin(c_test)].sample(frac=1).reset_index(drop=True)
        print('train',len(self.train_data),'valid',len(self.valid_data))
    def write_data(self):
        train =[[x,y] for x,y in zip(self.train_data['source'].tolist(), self.train_data['target'].tolist())]
        test = [[x,y] for x,y in zip(self.test_data['source'].tolist(), self.test_data['target'].tolist())]
        valid = [[x,y] for x,y in zip(self.valid_data['source'].tolist(), self.valid_data['target'].tolist())]
        r = {'train':train,'valid':valid,'test':test}
        import json
        with open('autocloze.json','w') as f:
            json.dump(r, f,ensure_ascii=False,indent=4)
if __name__ == '__main__':
    graber = AutoclozeGrabberGenGpt('../../../40-pn-dataset-auto.csv')
    graber.write_data()
