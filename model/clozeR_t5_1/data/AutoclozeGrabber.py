...
class ClozeRgrabber(AutoclozeGrabber):
    ...
    def __init__(self,train_path,valid_path,test_path):
        ...
        for temp in [self.train_data,self.valid_data,self.test_data]:
            ...
            temp['source_false'] = temp.apply(lambda x: '[SEP]'.join([x.context,x['False'],x['True']]),axis =1)
            temp['target_false'] = 1
        self.train_data = self.merge_tf(self.train_data).sample(frac=1).reset_index(drop=True)
        self.valid_data = self.merge_tf(self.valid_data).sample(frac=1).reset_index(drop=True)
        self.test_data = self.merge_tf(self.test_data).sample(frac=1).reset_index(drop=True)
        print(len(self.train_data)) 


if __name__ == '__main__':
    path = '../../../99-dataset/ClozeR/'
    graber = ClozeRgrabber(path + 'ClozeR_train.csv',path + 'ClozeR_valid.csv',path + 'ClozeR_test.csv')
