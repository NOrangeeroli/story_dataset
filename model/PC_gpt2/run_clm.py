...
def main():
    ...
    with open(data_args.train_file) as fin:
        datasets["train"] = [line.strip()+"<|endoftext|>" for line in fin]
    with open(data_args.validation_file) as fin:
        datasets["validation"] = [line.strip()+"<|endoftext|>" for line in fin]
    ...
    def preprocess_function(examples):
        result = {}
        result["input_ids"] = tokenizer([l for l in examples], truncation=True, padding=True, max_length=1024)
        result["labels"] = [ids  for ids in result["input_ids"].input_ids]
        return result
