...
class AutoclozeGrabber:
    ...
    def write_data(self):
        with open('data_train/train.txt','w') as f:
            for index,row in self.train_data.iterrows():
                f.write(row['data']+'\n')
        with open('data_train/valid.txt','w') as f:
            for index,row in self.valid_data.iterrows():
                f.write(row['data']+'\n')
        with open('data_train/test.txt','w') as f:
...
class outgengrabber(AutoclozeGrabber):
    def __init__(self,train_path,valid_path,test_path):
        self.train_data = pd.read_csv(train_path).sample(frac=1).reset_index(drop=True)
        self.valid_data = pd.read_csv(valid_path).sample(frac=1).reset_index(drop=True)
        self.test_data = pd.read_csv(test_path).sample(frac=1).reset_index(drop=True)
        for temp in [self.train_data,self.valid_data,self.test_data]:
            temp['source'] = temp['keywords']
            temp['target'] = temp['story']
            temp['data'] = temp.apply(lambda x: x.source.replace(',','#')+'[SEP]'+x.target,axis = 1)
 
class PCgrabber(AutoclozeGrabber):
    def __init__(self,train_path,valid_path,test_path):
        self.train_data = pd.read_csv(train_path)
        self.valid_data = pd.read_csv(valid_path)
        self.test_data = pd.read_csv(test_path)
        for temp in [self.train_data,self.valid_data,self.test_data]:
            temp['source'] = temp['context']
            temp['target'] = temp['True']
            temp['data'] = temp.apply(lambda x: x.source.replace('<MASK>','[MASK]')+'[SEP]'+x.target,axis = 1)
 


if __name__ == '__main__':
    path = '../../../99-dataset/PC/'
    graber = PCgrabber(path + 'PC_train.csv',path + 'PC_valid.csv',path + 'PC_test.csv')
    graber.write_data()
